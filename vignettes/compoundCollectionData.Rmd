...
processed by the `processDrugage` function in this package. 
he result DrugAge annotation table as well as the id-mapping table 
(DrugAge internal id to ChEMBL ID) were then
...
## CMAP SDF
The CMAP compound instance table was downloaded from [CMAP02](http://www.broadinstitute.org/cmap/cmap_instances_02.xls)
website and processed by the `buildCMAPdb` function
in this package.The result `cmap.db` contains both compound annotation and 
structure information. Since the annotation table only contains PubChem CID, 
the ChEMBL ids were added via PubChem CID to ChEMBL id mappings from 
[UniChem](ftp://ftp.ebi.ac.uk/pub/databases/chembl/UniChem/data/wholeSourceMapping/src_id1/src1src22.txt.gz).
The CMAP internal IDs were made for ChEMBL id to CMAP id mappings.
